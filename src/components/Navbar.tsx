    ...
    <nav className="p-4 lg:p-5 border-b border-gray-200 bg-white">
      <div className="flex flex-row justify-between gap-10">
        <div className="flex flex-row gap-10">
          {/* logo */}
          <img
            src="https://ik.imagekit.io/wq68aygdr/attio/attio_logo.png"
            alt="attio_logo"
          />
        ...
        <button
          ...
          className="lg:hidden rounded-full text-2xl hover:bg-gray-300"
        >
          {isOpen ? <X /> : <Menu />}
        </button>
      </div>
          ...
          <div>
            {items.map((item) => (
              <a
                href="#"
                key={item.title}
                className="block text-base border-b border-gray-300 p-4"
              >
                {item.title}
              </a>
            ))}
